...
public class TestPlanListAdapter extends BaseAdapter implements Filterable {
    ...
    public TestPlanListAdapter(Context context, TestPlanInfo[] objects) {
        ...
        this.mItems = new LinkedList<TestPlanInfo>(Arrays.asList(objects));
    ...
    private class PlanListFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (null == mOrigItems) {
                synchronized (mLock) {
                    mOrigItems = new LinkedList<TestPlanInfo>(mItems);
                }
            }
            if (null == constraint || constraint.length() == 0) {
                ArrayList<TestPlanInfo> list;
                synchronized (mLock) {
                    list = new ArrayList<TestPlanInfo>(mOrigItems);
                }
                ...
                List<TestPlanInfo> founded = new ArrayList<TestPlanInfo>();
