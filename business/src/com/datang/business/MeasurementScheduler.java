...
import com.datang.business.util.PhoneUtils;
...
public class MeasurementScheduler extends Service {
    ...
    public static final String TAG = "MeasurementScheduler";
    // This arbitrary id is private to MIOU
...
//  private PendingIntent checkinRetryIntentSender;
    ...
    /* Returns a IBinder that contains the instance of the com.datang.business.MeasurementScheduler object
...
//  private void startMIOUInForeGround() {
//    Logger.d("Service startMIOUInForeGround called");
//    //The intent to launch when the user clicks the expanded notification
//    Intent intent = new Intent(this, MiouApp.class);
...
//    notice.setLatestEventInfo(this, "MIOU",
        ...
        try {
            ...
            if (task != null && task.timeFromExecution() <= 0) {
                ...
                if (task.getDescription().priority == MeasurementTask.USER_PRIORITY) {
                    ...
                    future = measurementExecutor.submit(new BatteryCapPowerManager.PowerAwareTask(task, powerManager, this));
                }
...
//        }
            ...
            Logger.e("Exception when handling com.datang.business.measurements", e);
...
//  }
    ...
    /**
     * Submit a com.datang.business.MeasurementTask to the scheduler. Caller of this method can broadcast
     * an intent with MEASUREMENT_ACTION to start the measurement immediately.
     */
    public boolean submitTask(MeasurementTask task) {
        try {
            // Immediately handles com.datang.business.measurements created by user
            if (task.getDescription().priority == MeasurementTask.USER_PRIORITY) {
...
//
//    //This is deprecated in 3.x. But most phones still run 2.x systems
//    notice.setLatestEventInfo(this, "MIOU", notificationMsg, pendIntent);
//
//
...
//                  // Don't do anything with this - no need to report skipped com.datang.business.measurements
...
//    @Override
//    public void run() {
//      Logger.i("checking MIOU service for new tasks");
...
//  }
    ...
    private boolean userConsented() {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(
                getApplicationContext());
        boolean consented = prefs.getBoolean(Config.PREF_KEY_CONSENTED, true);
...
//  }
     ...
     * Needed for activities to monitor the progress of user com.datang.business.measurements.
     */
