...
class ConfigHandler implements IHandler {
  ...
  static ExecutorService Plan = Executors.newSingleThreadExecutor(new ThreadFactory() {
   ...
   * <p/>
   * Configuration response 配置响应 [Response] Command=Config Session=Session ID Cver=config version
   * #平台的最新配置版本 Result=AC/NAC Code=A Code #如果结果为NAC时的处理结果代码 PacketCount=N #将配置更新包拆分成N个包下发 PacketNo=n
   * #当前是第n个包（n=1,2,……）
   * <p/>
   * Configuration command 配置指令 Command=Config Session=Session ID Cver=config version #平台的最新配置版本
   * PacketCount=N #将配置更新包拆分成N个包下发 PacketNo=n #当前是第n个包 （n=1,2,……）
   */
